...
for i in range(2575):
    if i == 0:
      ...
      Camuon_HVP.append(2*alpha**2/(3*np.pi**2)*(K[i] / root_s[i] * w_amuon))
    elif i == 2574:
...
for i in range(2575):
    if i == 0:
      ...
      Calpha_had.append((2*alpha*(Mz**2)/(3*np.pi))*(w_alpha / (root_s[i] * (Mz**2 - root_s[i]**2))))
    elif i == 2574:
